/* minuit.f -- translated by f2c (version 20160102).
	...
	integer *i2, integer *i3)
{
    ...
    static char cwhyxt[40] = "FOR UNKNOWN REASONS                     ";
    ...
    extern /* Subroutine */ int mnread_(S_fp, integer *, integer *, U_fp), 
	    mncler_(void), mninit_(integer *, integer *, integer *);
...
/*                              . . . . . . verify FCN not time-dependent */
    ...
    mn7cnv_1.nfcn = 3;
L300:
    mn7min_1.fval3 = mn7min_1.amin * 2.f + 1.f;
/*                                   . . . . . . . . . . . read commands */
	...
	ierrf, U_fp futil)
{
    ...
    static char clabel[20] = "0123456789ABCDEFGHIJ";
    ...
    extern /* Subroutine */ int mnhess_(S_fp, U_fp), mnwerr_(void);
...
/*                 prepare this line's background pattern for contour */
	...
	*(unsigned char *)&chln[ixmid - 1] = '*';
	...
	ylabel = mn7ext_1.u[*ke2 - 1] + bwidy * .5f;
	...
	futil, ftnlen crdbin_len)
{
    ...
    static char clower[26] = "abcdefghijklmnopqrstuvwxyz";
    static char cupper[26] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
/*               ..................... save some values before fixing */
    ...
    dc = mn7min_1.dcovar;
    mn7min_1.apsi = mn7min_1.epsi * .5f;
    abest = mn7min_1.amin;
    mpar = mn7npr_1.npar;
	...
	integer *ierr, integer *isyswr, ftnlen crdbuf_len, ftnlen comand_len)
{
    ...
    static char cnull[15] = ")NULL STRING   ";
    static char cnumer[13] = "123456789-.0+";
	...
	futil)
{
    ...
    static char charal[28] = " .ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
/* debug printout: */
    ...
    *(unsigned char *)&mn7cpt_1.chpt[ipt - 1] = *(unsigned char *)&charal[ipt 
    ...
    flsb[0] = fnext;
/* Computing MAX */
    d__1 = fnext, d__2 = aminsv + mn7min_1.up * .1f;
    fnext = max(d__1,d__2);
    *aopt = sqrt(mn7min_1.up / (fnext - aminsv)) - 1.f;
...
/* Subroutine */ int mncuve_(S_fp fcn, U_fp futil)
{
    ...
    extern /* Subroutine */ int mndxdi_(doublereal *, integer *, doublereal *)
	    ...
	    , char *, ftnlen, ftnlen, ftnlen), mnwerr_(void);
...
/* Computing 2nd power */
		...
		mn7var_1.vhmat[ndex - 1] = 2.f / mn7der_1.g2[i__ - 1];
/* L555: */
	    }
	    mn7flg_1.isw[1] = 1;
	    mn7min_1.dcovar = 1.f;
...
/* Subroutine */ int mnderi_(S_fp fcn, U_fp futil)
{
    ...
    static doublereal tlrgrd, epspri, tlrstp, optstp, stpmax, stpmin;
    ...
    static cilist io___256 = { 0, 0, 0, "(/'  FIRST DERIVATIVE DEBUG PRINTOU"
	    ...
	    "EP ',               ' D1-D2    2ND DRV')", 0 };
    static cilist io___273 = { 0, 0, 0, fmt_41, 0 };
    static icilist io___274 = { 0, cbf1, 0, "(2E11.3)", 22, 1 };
...
/* Writing concatenation */
    ...
    dfmin = mn7cns_1.epsma2 * 8.f * (abs(mn7min_1.amin) + mn7min_1.up);
...
/* Computing MAX */
	    d__2 = optstp, d__3 = (d__1 = mn7der_1.gstep[i__ - 1] * .1f, abs(
		    d__1));
	    step = max(d__2,d__3);
/*                 but if parameter has limits, max step size = 0.5 */
	    if (mn7der_1.gstep[i__ - 1] < 0. && step > .5f) {
		step = .5f;
	    }
/*                 and not more than ten times the previous step */
	    stpmax = (d__1 = mn7der_1.gstep[i__ - 1], abs(d__1)) * 10.f;
	    if (step > stpmax) {
		step = stpmax;
	    }
/*                 minimum step size allowed by machine precision */
	    stpmin = (d__1 = mn7cns_1.epsma2 * mn7int_1.x[i__ - 1], abs(d__1))
		     * 8.f;
	    if (step < stpmin) {
		step = stpmin;
	    }
...
/*         take step negative */
	    ...
	    mn7der_1.grd[i__ - 1] = (fs1 - fs2) / (step * 2.f);
/* Computing 2nd power */
	    d__1 = step;
	    mn7der_1.g2[i__ - 1] = (fs1 + fs2 - mn7min_1.amin * 2.f) / (d__1 *
		     d__1);
	    mn7int_1.x[i__ - 1] = xtf;
	    if (ldebug) {
		d1d2 = (fs1 + fs2 - mn7min_1.amin * 2.f) / step;
		io___273.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___273);
...
L100:
	...
	goto L150;
L120:
	dd = (mn7ext_1.blim[iext - 1] - mn7ext_1.alim[iext - 1]) * .5f * cos(
		mn7int_1.x[iint - 1]);
...
/* Subroutine */ int mnemat_(doublereal *emat, integer *ndim)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___299 = { 0, 0, 0, "(/A,I4,A,I3,A,G10.3)", 0 };
    static cilist io___301 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___303 = { 0, 0, 0, "(A)", 0 };
    static cilist io___313 = { 0, 0, 0, "(1X,13E10.3)", 0 };
...
/* C        to be called by user, who must dimension EMAT at (NDIM,NDIM) */
    ...
    if (mn7flg_1.isw[4] >= 2) {
	io___299.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___299);
...
/*                    size of matrix to be printed */
	...
	npard = *ndim;
	if (mn7flg_1.isw[4] >= 0) {
	    io___301.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___301);
...
/*                 NPERLN is the number of elements that fit on one line */
	...
	io___303.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___303);
...
/*                    IZ is number of columns to be printed in row I */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		io___313.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___313);
		i__4 = k2;
		for (kk = k; kk <= i__4; ++kk) {
	...
	integer *llist, integer *ierflg, U_fp futil, ftnlen comand_len)
{
    ...
    static char clower[26] = "abcdefghijklmnopqrstuvwxyz";
    static char cupper[26] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    static char cname[10*40] = "MINImize  " "SEEk      " "SIMplex   " "MIGra"
    ...
    extern /* Subroutine */ int stand_(void);
    ...
    extern /* Subroutine */ int mnseek_(S_fp, U_fp), mnsimp_(S_fp, U_fp), 
	    mnmigr_(S_fp, U_fp), mnwerr_(void);
    ...
    extern /* Subroutine */ int mncuve_(S_fp, U_fp), mnmnos_(S_fp, U_fp), 
	    ...
	    S_fp, U_fp), mnmatu_(integer *), mnsave_(void), mnimpr_(S_fp, 
	    U_fp);
    static integer nowprt;
    extern /* Subroutine */ int mncler_(void), mnhelp_(char *, integer *, 
	    ftnlen), mncont_(S_fp, integer *, integer *, integer *, 
	    doublereal *, doublereal *, integer *, U_fp), mninex_(doublereal *
	    ), mnamin_(S_fp, U_fp);

    /* Fortran I/O blocks */
    static cilist io___329 = { 0, 0, 0, fmt_25, 0 };
    static icilist io___334 = { 0, cvblnk, 0, "(I2)", 2, 1 };
    static cilist io___336 = { 0, 0, 0, c26, 0 };
    static cilist io___337 = { 0, 0, 0, "(1H ,10(1H*))", 0 };
    static cilist io___338 = { 0, 0, 0, "(1H ,10(1H*),A,I3,A)", 0 };
    static cilist io___339 = { 0, 0, 0, "(11X,'UNKNOWN COMMAND IGNORED:',A)", 
	    0 };
    static cilist io___342 = { 0, 0, 0, "(/' TOO MANY FUNCTION CALLS. MINOS "
	    "GIVES UP'/)", 0 };
    static cilist io___343 = { 0, 0, 0, "(1H1)", 0 };
    static cilist io___347 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___354 = { 0, 0, 0, "(A,I4,A,A)", 0 };
    static cilist io___356 = { 0, 0, 0, "(A,I4)", 0 };
    static cilist io___358 = { 0, 0, 0, "(A,I4,A)", 0 };
    static cilist io___361 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___367 = { 0, 0, 0, "(/A/)", 0 };
    static cilist io___368 = { 0, 0, 0, "(A)", 0 };
    static cilist io___377 = { 0, 0, 0, "(10X,A)", 0 };
    static cilist io___378 = { 0, 0, 0, "(A)", 0 };
    static cilist io___380 = { 0, 0, 0, fmt_3101, 0 };
...
/* L20: */
    ...
    if (s_cmp(mn7tit_1.cword, "SET PRI", (ftnlen)7, (ftnlen)7) != 0 || 
	    mn7arg_1.word7[0] >= 0.f) {
	if (mn7flg_1.isw[4] >= 0) {
	    lnow = *llist;
	    if (lnow > 4) {
		lnow = 4;
	    }
	    io___329.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___329);
...
/* Writing concatenation */
		...
		io___336.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___336);
		i__1 = kll;
...
L510:
    ...
    io___342.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___342);
    e_wsfe();
    *ierflg = 4;
    goto L5000;
...
L901:
    ...
    lfixed = FALSE_;
    if (*llist == 0) {
	io___347.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___347);
	...
	goto L950;
L930:
	io___354.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___354);
...
L1000:
    ...
    goto L5000;
L1005:
    io___356.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___356);
...
L1210:
    ...
    goto L5000;
L1250:
    io___358.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___358);
    do_fio(&c__1, " PARAMETER", (ftnlen)10);
    do_fio(&c__1, (char *)&iext, (ftnlen)sizeof(integer));
...
L1300:
	...
	if (mn7npr_1.npar == 2) {
	    ...
	    io___361.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___361);
	    do_fio(&c__1, mn7tit_1.cword, (ftnlen)20);
	    do_fio(&c__1, ":  NO PARAMETERS REQUESTED ", (ftnlen)27);
	    e_wsfe();
...
L1900:
	...
	iflag = 3;
	io___367.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___367);
...
/*                                      . . . . . . . . . . blank line */
L3300:
    io___377.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___377);
    do_fio(&c__1, " BLANK COMMAND IGNORED.", (ftnlen)23);
    e_wsfe();
    *ierflg = 1;
...
L3100:
    io___380.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___380);
    do_fio(&c__1, mn7tit_1.cword, (ftnlen)20);
...
/* Subroutine */ int mnfixp_(integer *iint, integer *ierr)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___384 = { 0, 0, 0, "(A,I4)", 0 };
    static cilist io___386 = { 0, 0, 0, "(A,I4,A,I4)", 0 };
...
/*                           first see if it can be done */
	...
	*ierr = 1;
	io___384.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___384);
	...
	*ierr = 1;
	io___386.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___386);
...
/* Subroutine */ int mnfree_(integer *k)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___399 = { 0, 0, 0, fmt_510, 0 };
    static cilist io___400 = { 0, 0, 0, fmt_500, 0 };
    static cilist io___402 = { 0, 0, 0, fmt_540, 0 };
    static cilist io___404 = { 0, 0, 0, fmt_530, 0 };
    static cilist io___417 = { 0, 0, 0, fmt_520, 0 };
...
/* --       IS = internal number of parameter being restored */
    if (*k > 1) {
	io___399.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___399);
	e_wsfe();
    }
    if (mn7fx1_1.npfix < 1) {
...
/*                   release parameter with specified external number */
    ...
    io___402.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___402);
    e_wsfe();
    return 0;
L15:
...
L21:
    io___404.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___404);
...
/* L100: */
	...
	io___417.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___417);
...
/* Subroutine */ int mngrad_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___423 = { 0, 0, 0, fmt_51, 0 };
    static cilist io___428 = { 0, 0, 0, fmt_99, 0 };
    static cilist io___429 = { 0, 0, 0, "(A)", 0 };
    static cilist io___430 = { 0, 0, 0, fmt_1003, 0 };
...
/*                    get MINUIT-calculated first derivatives */
    ...
    io___423.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___423);
    e_wsfe();
    mn7flg_1.isw[2] = 1;
    lnone = FALSE_;
	...
	io___428.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___428);
	do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
...
/* Subroutine */ int mnhelp_(char *comd, integer *lout, ftnlen comd_len)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___431 = { 0, 0, 0, fmt_10000, 0 };
    static cilist io___432 = { 0, 0, 0, fmt_10001, 0 };
    static cilist io___434 = { 0, 0, 0, fmt_10100, 0 };
    static cilist io___435 = { 0, 0, 0, fmt_10200, 0 };
    static cilist io___436 = { 0, 0, 0, fmt_10300, 0 };
    static cilist io___437 = { 0, 0, 0, fmt_10400, 0 };
    static cilist io___438 = { 0, 0, 0, fmt_10500, 0 };
    static cilist io___439 = { 0, 0, 0, fmt_10600, 0 };
    static cilist io___440 = { 0, 0, 0, fmt_10700, 0 };
    static cilist io___441 = { 0, 0, 0, fmt_10800, 0 };
    static cilist io___442 = { 0, 0, 0, fmt_10900, 0 };
    static cilist io___443 = { 0, 0, 0, fmt_11000, 0 };
    static cilist io___444 = { 0, 0, 0, fmt_11100, 0 };
    static cilist io___445 = { 0, 0, 0, fmt_11150, 0 };
    static cilist io___446 = { 0, 0, 0, fmt_11200, 0 };
    static cilist io___447 = { 0, 0, 0, fmt_11300, 0 };
    static cilist io___448 = { 0, 0, 0, fmt_11400, 0 };
    static cilist io___449 = { 0, 0, 0, fmt_11500, 0 };
    static cilist io___450 = { 0, 0, 0, fmt_11600, 0 };
    static cilist io___451 = { 0, 0, 0, fmt_11700, 0 };
    static cilist io___452 = { 0, 0, 0, fmt_11800, 0 };
    static cilist io___453 = { 0, 0, 0, fmt_11801, 0 };
    static cilist io___454 = { 0, 0, 0, fmt_11802, 0 };
    static cilist io___455 = { 0, 0, 0, fmt_11803, 0 };
    static cilist io___456 = { 0, 0, 0, fmt_11804, 0 };
    static cilist io___457 = { 0, 0, 0, fmt_11805, 0 };
    static cilist io___458 = { 0, 0, 0, fmt_11806, 0 };
    static cilist io___459 = { 0, 0, 0, fmt_11807, 0 };
    static cilist io___460 = { 0, 0, 0, fmt_11808, 0 };
    static cilist io___461 = { 0, 0, 0, fmt_11809, 0 };
    static cilist io___462 = { 0, 0, 0, fmt_11810, 0 };
    static cilist io___463 = { 0, 0, 0, fmt_11811, 0 };
    static cilist io___464 = { 0, 0, 0, fmt_11812, 0 };
    static cilist io___465 = { 0, 0, 0, fmt_11813, 0 };
    static cilist io___466 = { 0, 0, 0, fmt_11814, 0 };
    static cilist io___467 = { 0, 0, 0, fmt_11815, 0 };
    static cilist io___468 = { 0, 0, 0, fmt_11816, 0 };
    static cilist io___469 = { 0, 0, 0, fmt_11817, 0 };
    static cilist io___470 = { 0, 0, 0, fmt_11900, 0 };
    static cilist io___471 = { 0, 0, 0, fmt_11901, 0 };
    static cilist io___472 = { 0, 0, 0, fmt_11902, 0 };
    static cilist io___473 = { 0, 0, 0, fmt_11903, 0 };
    static cilist io___474 = { 0, 0, 0, fmt_11904, 0 };
    static cilist io___475 = { 0, 0, 0, fmt_12000, 0 };
    static cilist io___476 = { 0, 0, 0, fmt_12100, 0 };
    static cilist io___477 = { 0, 0, 0, fmt_12200, 0 };
    static cilist io___478 = { 0, 0, 0, fmt_12300, 0 };
    static cilist io___479 = { 0, 0, 0, fmt_13000, 0 };
...
/* Subroutine */ int mnhess_(S_fp fcn, U_fp futil)
{
    ...
    static integer id;
    static doublereal yy[50], g2i, fs1, fs2, sag, xtf, xti, xtj;
    static char cbf1[22];
    ...
    extern /* Subroutine */ int mnpsdf_(void), mnvert_(doublereal *, integer *
	    , integer *, integer *, integer *);

    /* Fortran I/O blocks */
    static cilist io___484 = { 0, 0, 0, "(A)", 0 };
    static icilist io___490 = { 0, cbf1, 0, "(G12.3)", 12, 1 };
    static cilist io___491 = { 0, 0, 0, "(A,A)", 0 };
    static icilist io___498 = { 0, cbf1, 0, "(I4)", 4, 1 };
    static icilist io___508 = { 0, cbf1, 0, "(I4)", 4, 1 };
    static cilist io___510 = { 0, 0, 0, fmt_31, 0 };
    static icilist io___514 = { 0, cbf1, 0, "(I2,2E10.2)", 22, 1 };
    static cilist io___522 = { 0, 0, 0, "(A)", 0 };
    static cilist io___523 = { 0, 0, 0, "(A)", 0 };
...
/* C */
	...
	io___484.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___484);
...
/* Writing concatenation */
    ...
    mn7min_1.amin = fs1;
    if (ldebug) {
	io___491.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___491);
...
/* Computing 2nd power */
	...
	xtf = mn7int_1.x[i__ - 1];
	dmin__ = mn7cns_1.epsma2 * 8.f * abs(xtf);
...
/*           take two steps */
		...
		++mn7cnv_1.nfcn;
		mn7int_1.x[i__ - 1] = xtf;
		sag = (fs1 + fs2 - mn7min_1.amin * 2.f) * .5f;
		if (sag != 0.) {
		    goto L30;
		}
...
/* Computing 2nd power */
	    d__1 = d__;
	    mn7der_1.g2[i__ - 1] = sag * 2.f / (d__1 * d__1);
	    mn7der_1.grd[i__ - 1] = (fs1 - fs2) / (d__ * 2.f);
	    if (ldebug) {
		io___510.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___510);
		do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	    ...
	    yy[i__ - 1] = fs1;
	    dlast = d__;
	    d__ = sqrt(aimsag * 2.f / (d__1 = mn7der_1.g2[i__ - 1], abs(d__1))
		    );
...
/*                  verify matrix positive-definite */
	...
	for (j = 1; j <= i__3; ++j) {
	    ndex = i__ * (i__ - 1) / 2 + j;
	    mn7sim_1.p[i__ + j * 50 - 51] = mn7var_1.vhmat[ndex - 1];
/* L219: */
	    mn7sim_1.p[j + i__ * 50 - 51] = mn7sim_1.p[i__ + j * 50 - 51];
	}
/* L220: */
    }
...
/*                              off-diagonal elements */
	...
	for (j = 1; j <= i__3; ++j) {
	    ++ndex;
	    ztemp = mn7sim_1.p[i__ + j * 50 - 51] * 2.f;
...
/*                              diagonal elements */
	++ndex;
	mn7var_1.vhmat[ndex - 1] = mn7sim_1.p[i__ + i__ * 50 - 51] * 2.f;
/* Computing 2nd power */
	d__1 = mn7der_1.grd[i__ - 1];
	mn7min_1.edm += mn7sim_1.p[i__ + i__ * 50 - 51] * (d__1 * d__1);
/* L230: */
    }
    if (mn7flg_1.isw[4] >= 1 && mn7flg_1.isw[1] == 3 && mn7cnv_1.itaur == 0) {
	io___522.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___522);
...
L390:
    ...
    if (mn7flg_1.isw[4] >= 0) {
	io___523.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___523);
...
/* Subroutine */ int mnhes1_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___544 = { 0, 0, 0, fmt_11, 0 };
    static icilist io___547 = { 0, cbf1, 0, "(2G11.3)", 22, 1 };
...
/* C         and appropriate step sizes GSTEP */
    ...
    idrv = 1;
    nparx = mn7npr_1.npar;
    dfmin = mn7cns_1.epsma2 * 4.f * (abs(mn7min_1.amin) + mn7min_1.up);
/*                                     main loop over parameters */
    i__1 = mn7npr_1.npar;
    for (i__ = 1; i__ <= i__1; ++i__) {
	xtf = mn7int_1.x[i__ - 1];
	dmin__ = mn7cns_1.epsma2 * 4.f * abs(xtf);
	epspri = mn7cns_1.epsma2 + (d__1 = mn7der_1.grd[i__ - 1] * 
		mn7cns_1.epsma2, abs(d__1));
	optstp = sqrt(dfmin / ((d__1 = mn7der_1.g2[i__ - 1], abs(d__1)) + 
		epspri));
	d__ = (d__1 = mn7der_1.gstep[i__ - 1], abs(d__1)) * .2f;
	if (d__ > optstp) {
	    d__ = optstp;
	}
	if (d__ < dmin__) {
...
/*                                       check if step sizes appropriate */
	    ...
	    if (ldebug) {
		io___544.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___544);
		do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
		do_fio(&c__1, (char *)&idrv, (ftnlen)sizeof(integer));
		do_fio(&c__1, (char *)&mn7der_1.gstep[i__ - 1], (ftnlen)
...
/* Subroutine */ int mnimpr_(S_fp fcn, U_fp futil)
{
    ...
    static integer jl;
    static doublereal wg, xi, reg, sig2, amax, dsav[50];
    ...
    /* Fortran I/O blocks */
    static cilist io___563 = { 0, 0, 0, fmt_1040, 0 };
    static cilist io___576 = { 0, 0, 0, fmt_1000, 0 };
    static cilist io___577 = { 0, 0, 0, fmt_1030, 0 };
    static cilist io___578 = { 0, 0, 0, fmt_1020, 0 };
    static cilist io___579 = { 0, 0, 0, fmt_1010, 0 };
...
/*               Save inverted matrix in VT */
	...
	for (j = 1; j <= i__1; ++j) {
	    ++ndex;
/* L12: */
	    mn7vat_1.vthmat[ndex - 1] = mn7sim_1.p[i__ + j * 50 - 51];
	}
    }
    loop = 0;
...
L20:
    i__1 = mn7npr_1.npar;
    for (i__ = 1; i__ <= i__1; ++i__) {
	mn7int_1.dirin[i__ - 1] = dsav[i__ - 1] * 2.f;
	mnrn15_(&rnum, &iseed);
/* L25: */
    ...
    reg = 2.f;
    if (mn7flg_1.isw[4] >= 0) {
	io___563.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___563);
...
/*                                        . . . . set up  random simplex */
	...
	xi = mn7int_1.x[i__ - 1];
	mnrn15_(&rnum, &iseed);
	mn7int_1.x[i__ - 1] = xi - mn7int_1.dirin[i__ - 1] * (rnum - .5f);
	mncalf_((S_fp)fcn, mn7int_1.x, &ycalf, (U_fp)futil);
	y[i__ - 1] = ycalf;
	...
	for (j = 1; j <= i__2; ++j) {
/* L40: */
	    mn7sim_1.p[j + i__ * 50 - 51] = mn7int_1.x[j - 1];
	}
	mn7sim_1.p[i__ + nparp1 * 50 - 51] = xi;
	mn7int_1.x[i__ - 1] = xi;
/* L45: */
    }
...
/*         calculate new point * by reflection */
	...
	for (j = 1; j <= i__2; ++j) {
/* L59: */
	    pb += wg * mn7sim_1.p[i__ + j * 50 - 51];
	}
	mn7sim_1.pbar[i__ - 1] = pb - wg * mn7sim_1.p[i__ + jh * 50 - 51];
/* L60: */
...
L73:
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L74: */
	mn7sim_1.pstst[i__ - 1] = mn7sim_1.p[i__ + jh * 50 - 51] * .5 + 
...
L95:
    if (mn7flg_1.isw[4] >= 0) {
	io___576.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___576);
	e_wsfe();
    }
    reg = .1f;
...
L150:
    ...
    i__1 = mn7npr_1.npar;
    for (i__ = 1; i__ <= i__1; ++i__) {
	mn7int_1.dirin[i__ - 1] = dsav[i__ - 1] * .1f;
	if ((d__1 = mn7int_1.x[i__ - 1] - mn7int_1.xt[i__ - 1], abs(d__1)) > 
		mn7int_1.dirin[i__ - 1]) {
	    goto L250;
	}
...
L325:
    i__1 = mn7npr_1.npar;
    for (i__ = 1; i__ <= i__1; ++i__) {
	mn7int_1.dirin[i__ - 1] = dsav[i__ - 1] * .01f;
/* L330: */
	mn7int_1.x[i__ - 1] = mn7int_1.xt[i__ - 1];
    }
...
/* C        internal coordinates call this routine before calling FCN. */
	...
	if (mn7inx_1.nvarl[i__ - 1] == 1) {
	    ...
	    mn7ext_1.u[i__ - 1] = mn7ext_1.alim[i__ - 1] + (sin(pint[j]) + 
		    1.f) * .5f * (mn7ext_1.blim[i__ - 1] - mn7ext_1.alim[i__ 
		    - 1]);
	}
/* L100: */
    }
...
/* Subroutine */ int mninit_(integer *i1, integer *i2, integer *i3)
{
    ...
    extern /* Subroutine */ int mncler_(void), mntiny_(doublereal *, 
	    doublereal *);
    static doublereal epstry, distnn;

    /* Fortran I/O blocks */
    static cilist io___588 = { 0, 0, 0, "(A,A,E10.2)", 0 };
...
/*            I/O unit numbers */
    ...
    mn7io2_1.nstkrd = 0;
/*               version identifier */
    s_copy(mn7tit_1.cvrsn, "2001.5", (ftnlen)6, (ftnlen)6);
/*               some CONSTANT constants in COMMON */
    mn7npr_1.maxint = 50;
    mn7npr_1.maxext = 100;
    mn7cns_1.undefi = -54321.f;
    mn7cns_1.bigedm = 123456.f;
...
/* L33: */
    ...
    epstry = 1e-7f;
    mn7cns_1.epsmac = epstry * 4.f;
    io___588.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___588);
...
/* Subroutine */ int mninpu_(integer *iunit, integer *ierr)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___591 = { 0, 0, 0, "(A)", 0 };
...
/*                              IUNIT = 0, revert to previous input file */
	...
	if (mn7io2_1.nstkrd == 0) {
	    io___591.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___591);
...
/* Subroutine */ int mnintr_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___596 = { 0, 0, 0, "(2A/)", 0 };
...
/* C */
    ...
    io___596.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___596);
...
/* Subroutine */ int mnlims_(void)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___601 = { 0, 0, 0, "(11X,A,I3)", 0 };
    static cilist io___602 = { 0, 0, 0, fmt_134, 0 };
    static cilist io___605 = { 0, 0, 0, fmt_237, 0 };
    static cilist io___606 = { 0, 0, 0, "(A,I3,A)", 0 };
    static cilist io___607 = { 0, 0, 0, "(A,I3)", 0 };
    static cilist io___609 = { 0, 0, 0, "(A)", 0 };
    static cilist io___610 = { 0, 0, 0, fmt_134, 0 };
    static cilist io___611 = { 0, 0, 0, "(A,I3)", 0 };
...
/*             see if parameter has been fixed */
	if (kint <= 0) {
	    if (mn7flg_1.isw[4] >= 0) {
		io___601.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___601);
...
/*            remove limits from parameter */
	    if (mn7flg_1.isw[4] > 0) {
		io___602.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___602);
		do_fio(&c__1, (char *)&inu, (ftnlen)sizeof(integer));
		e_wsfe();
	    }
...
/*             put limits on parameter */
	    ...
	    if (mn7flg_1.isw[4] > 0) {
		io___605.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___605);
		do_fio(&c__1, (char *)&inu, (ftnlen)sizeof(integer));
...
L30:
	...
	io___606.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___606);
...
/*                                       remove limits */
	...
	if (mn7flg_1.isw[4] > 0) {
	    io___610.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___610);
	    do_fio(&c__1, (char *)&i2, (ftnlen)sizeof(integer));
	    e_wsfe();
	}
	...
	doublereal *step, doublereal *slope, doublereal *toler, U_fp futil)
{
    ...
    static char charal[26] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    ...
    /* Fortran I/O blocks */
    static cilist io___619 = { 0, 0, 0, "(A/2E14.5/2X,10F10.5)", 0 };
    static cilist io___647 = { 0, 0, 0, "(A/(2X,6G12.4))", 0 };
    static cilist io___648 = { 0, 0, 0, "(' AFTER',I3,' POINTS,',A)", 0 };
...
/*                              debug check if start is ok */
	...
	++mn7cnv_1.nfcn;
	if (f1 != *fstart) {
	    io___619.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___619);
...
L65:
    s_copy(cmess, " LINE SEARCH HAS EXHAUSTED THE LIMIT OF FUNCTION CALLS ", (
	    ftnlen)60, (ftnlen)55);
    if (ldebug) {
	io___647.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___647);
...
/* L120: */
    ...
    if (ldebug) {
	io___648.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___648);
...
/* Subroutine */ int mnmatu_(integer *kode)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___650 = { 0, 0, 0, "(1X,A)", 0 };
    static cilist io___651 = { 0, 0, 0, "(' MNMATU: NPAR=0')", 0 };
    static cilist io___653 = { 0, 0, 0, "(1X,A)", 0 };
    static cilist io___656 = { 0, 0, 0, fmt_150, 0 };
    static cilist io___667 = { 0, 0, 0, fmt_171, 0 };
    static cilist io___671 = { 0, 0, 0, fmt_181, 0 };
    static cilist io___672 = { 0, 0, 0, "(1X,A)", 0 };
...
/* C */
    ...
    if (mn7npr_1.npar == 0) {
	io___651.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___651);
	e_wsfe();
	goto L500;
    }
...
/*     NCOEF is number of coeff. that fit on one line, not to exceed 20 */
    ...
    io___656.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___656);
    i__1 = nparm;
    for (id = 1; id <= i__1; ++id) {
...
/* L170: */
	...
	nparm = min(mn7npr_1.npar,ncoef);
	io___667.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___667);
...
/* Computing MIN */
	    ...
	    nparm = min(i__2,i__3);
	    io___671.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___671);
	    i__2 = nparm;
...
L200:
    ...
    if (isw2 < 3) {
	io___672.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___672);
	do_fio(&c__1, mn7tit_1.covmes + isw2 * 22, (ftnlen)22);
	e_wsfe();
    }
...
/* Subroutine */ int mnmigr_(S_fp fcn, U_fp futil)
{
    ...
    static doublereal fs, gs[50], ri, vg[50], gvg, vgi, xxs[50], gdel, gami;
    static integer npfn;
    static doublereal flnu[50];
    static integer vlen, ndex, iext, iter;
    static doublereal step[50], dsum, gssq, vsum;
    ...
    extern /* Subroutine */ int mnhess_(S_fp, U_fp), mnwerr_(void), mninex_(
	    doublereal *), mnwarn_(char *, char *, char *, ftnlen, ftnlen, 
	    ftnlen), mnprin_(integer *, doublereal *);
    static integer nrstrt;
    extern /* Subroutine */ int mnmatu_(integer *), mnpsdf_(void);
    static doublereal gdgssq;

    /* Fortran I/O blocks */
    static cilist io___683 = { 0, 0, 0, fmt_470, 0 };
    static cilist io___691 = { 0, 0, 0, "(A,I3,2G13.3)", 0 };
    static cilist io___694 = { 0, 0, 0, "(A,A/(1X,10G10.2))", 0 };
    static cilist io___708 = { 0, 0, 0, "(A,(1X,10G10.3))", 0 };
    static cilist io___712 = { 0, 0, 0, "(A,F5.1,A)", 0 };
    static cilist io___713 = { 0, 0, 0, "(A,(1X,10G10.3))", 0 };
    static cilist io___715 = { 0, 0, 0, "(A)", 0 };
...
/* C */
    ...
    npfn = mn7cnv_1.nfcn;
    nparx = mn7npr_1.npar;
    vlen = mn7npr_1.npar * (mn7npr_1.npar + 1) / 2;
    nrstrt = 0;
    npsdf = 0;
    lined2 = 0;
    mn7flg_1.isw[3] = -1;
    rhotol = mn7min_1.apsi * .001f;
    if (iswtr >= 1) {
	io___683.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___683);
...
/*                           do line search if second derivative negative */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    fs = mn7min_1.amin;
	    mnline_((S_fp)fcn, xxs, &fs, step, &gdel, &c_b1197, (U_fp)futil);
	    mnwarn_("D", "MNMIGR", "Negative G2 line search", (ftnlen)1, (
		    ftnlen)6, (ftnlen)23);
	    iext = mn7inx_1.nexofi[i__ - 1];
	    if (ldebug) {
		io___691.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___691);
...
/* L8: */
    ...
    mn7min_1.dcovar = 1.f;
    if (ldebug) {
	io___694.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___694);
...
/* Computing 2nd power */
	d__1 = gs[i__ - 1];
	mn7min_1.edm += d__1 * d__1 * .5f * mn7var_1.vhmat[ndex - 1];
    }
    mn7min_1.edm = mn7min_1.edm * .5f * (mn7min_1.dcovar * 3.f + 1.f);
    if (mn7min_1.edm < 0.) {
...
/* L100: */
	mn7min_1.edm += mn7der_1.grd[i__ - 1] * ri * .5f;
    }
    mn7min_1.edm = mn7min_1.edm * .5f * (mn7min_1.dcovar * 3.f + 1.f);
...
/*                                        .  update covariance matrix */
    ...
    if (ldebug) {
	io___708.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___708);
	...
	for (j = 1; j <= i__2; ++j) {
	    ...
	    dsum += abs(d__);
	    ndex = i__ * (i__ - 1) / 2 + j;
	    mn7var_1.vhmat[ndex - 1] += d__ * 2.f;
	    vsum += (d__1 = mn7var_1.vhmat[ndex - 1], abs(d__1));
/* L120: */
	}
    }
/*                smooth local fluctuations by averaging DCOVAR */
    mn7min_1.dcovar = (mn7min_1.dcovar + dsum / vsum) * .5f;
    if (iswtr >= 3 || ldebug) {
	io___712.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___712);
	do_fio(&c__1, " RELATIVE CHANGE IN COV. MATRIX=", (ftnlen)32);
	d__1 = mn7min_1.dcovar * 100.f;
	do_fio(&c__1, (char *)&d__1, (ftnlen)sizeof(doublereal));
	do_fio(&c__1, "%", (ftnlen)1);
	e_wsfe();
    }
    if (ldebug) {
	io___713.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___713);
...
/* L125: */
	...
	for (j = 1; j <= i__1; ++j) {
	    ndex = i__ * (i__ - 1) / 2 + j;
/* L130: */
	    mn7var_1.vhmat[ndex - 1] += gvg * 2.f * flnu[i__ - 1] * flnu[j - 
		    1];
	}
    }
L135:
/*                                              and see if converged */
    if (mn7min_1.edm < rhotol * .1f) {
	goto L300;
    }
...
/* L140: */
    ...
    fs = mn7min_1.amin;
    if (mn7flg_1.isw[1] == 0 && mn7min_1.dcovar < .5f) {
	mn7flg_1.isw[1] = 1;
    }
    if (mn7flg_1.isw[1] == 3 && mn7min_1.dcovar > .1f) {
	mn7flg_1.isw[1] = 1;
    }
    if (mn7flg_1.isw[1] == 1 && mn7min_1.dcovar < .05f) {
	mn7flg_1.isw[1] = 3;
    }
    goto L24;
/*                                        . . . . .  end main loop */
...
L200:
    if (iswtr >= 1) {
	io___716.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___716);
...
/* Subroutine */ int mnmnos_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___727 = { 0, 0, 0, fmt_564, 0 };
    static cilist io___732 = { 0, 0, 0, fmt_675, 0 };
    static cilist io___733 = { 0, 0, 0, "(A)", 0 };
...
/*                                      . loop over parameters requested */
	...
	if ((integer) mn7arg_1.word7[1] == 0) {
	    ...
	    io___727.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___727);
...
L650:
    ...
    io___732.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___732);
    e_wsfe();
    goto L900;
L700:
    io___733.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___733);
	...
	doublereal *val2pl, doublereal *val2mi, U_fp futil)
{
    ...
    /* Local variables */
    static integer i__, j;
    static doublereal w[50], dc;
    static integer it;
    static doublereal ut, du1, fac, gcc[50], sig, sav;
    ...
    /* Fortran I/O blocks */
    static cilist io___755 = { 0, 0, 0, "(A,I5,A,I5)", 0 };
    static cilist io___759 = { 0, 0, 0, fmt_806, 0 };
    static cilist io___764 = { 0, 0, 0, fmt_801, 0 };
    static cilist io___768 = { 0, 0, 0, fmt_808, 0 };
    static cilist io___769 = { 0, 0, 0, fmt_807, 0 };
    static cilist io___770 = { 0, 0, 0, fmt_802, 0 };
    static cilist io___771 = { 0, 0, 0, fmt_805, 0 };
    static cilist io___772 = { 0, 0, 0, "(5X, 74(1H*))", 0 };
...
/* L135: */
    ...
    mn7err_1.erp[it - 1] = 0.f;
    mn7err_1.ern[it - 1] = 0.f;
    mninex_(mn7int_1.xt);
    ut = mn7ext_1.u[*ilax - 1];
    if (mn7inx_1.nvarl[*ilax - 1] == 1) {
	mn7ext_1.alim[*ilax - 1] = ut - w[it - 1] * 100.f;
	mn7ext_1.blim[*ilax - 1] = ut + w[it - 1] * 100.f;
    }
    ndex = it * (it + 1) / 2;
...
/*                           fix the parameter in question */
    mnfixp_(&it, &ierr);
    if (ierr > 0) {
	io___755.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___755);
...
/*                                        . sig=sign of error being calcd */
	if (mn7flg_1.isw[4] > 1) {
	    io___759.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___759);
...
/* L185: */
	...
	if (mn7flg_1.isw[4] > 1) {
	    io___764.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___764);
	    do_fio(&c__1, (char *)&(*ilax), (ftnlen)sizeof(integer));
...
/*                                        loop to hit AMIN+UP */
	...
	if (abest - mn7min_1.amin > mn7min_1.up * .01f) {
	    goto L650;
	}
	if (iercr == 1) {
	    goto L440;
...
/*                                        . error successfully calculated */
	...
	if (mn7flg_1.isw[4] > 1) {
	    io___768.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___768);
	    do_fio(&c__1, csig, (ftnlen)4);
	    do_fio(&c__1, (char *)&(*ilax), (ftnlen)sizeof(integer));
	    do_fio(&c__1, mn7nam_1.cpnam + (*ilax - 1) * 10, (ftnlen)10);
...
L440:
	if (mn7flg_1.isw[4] >= 1) {
	    io___769.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___769);
	    do_fio(&c__1, csig, (ftnlen)4);
	    do_fio(&c__1, (char *)&(*ilax), (ftnlen)sizeof(integer));
	    do_fio(&c__1, mn7nam_1.cpnam + (*ilax - 1) * 10, (ftnlen)10);
	...
	cnamj_len)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___776 = { 0, 0, 0, fmt_9, 0 };
    static cilist io___779 = { 0, 0, 0, "(A)", 0 };
    static cilist io___780 = { 0, 0, 0, fmt_61, 0 };
    static cilist io___781 = { 0, 0, 0, fmt_82, 0 };
    static cilist io___783 = { 0, 0, 0, fmt_127, 0 };
    static cilist io___784 = { 0, 0, 0, fmt_132, 0 };
    static cilist io___785 = { 0, 0, 0, fmt_135, 0 };
    static cilist io___786 = { 0, 0, 0, "(/A,A/A/)", 0 };
    static icilist io___789 = { 0, chbufi, 0, "(I4)", 4, 1 };
...
/*                     parameter number exceeds allowed maximum value */
	io___776.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___776);
...
/* L40: */
	...
	if (kint >= mn7npr_1.maxint) {
	    io___779.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___779);
...
/*                                      . . .print heading */
	...
	io___780.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___780);
	e_wsfe();
...
/*                                         variable parameter with limits */
	...
	if (mn7flg_1.isw[4] >= 0) {
	    io___784.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___784);
	    do_fio(&c__1, (char *)&(*k), (ftnlen)sizeof(integer));
	    do_fio(&c__1, cnamk, (ftnlen)10);
	    do_fio(&c__1, (char *)&(*uk), (ftnlen)sizeof(doublereal));
...
/*                                       lists are now arranged . . . . */
	...
	mn7int_1.dirin[in - 1] = (abs(vplu) + abs(vminu)) * .5f;
/* Computing 2nd power */
	d__1 = mn7int_1.dirin[in - 1];
	mn7der_1.g2[in - 1] = mn7min_1.up * 2.f / (d__1 * d__1);
	gsmin = mn7cns_1.epsma2 * 8.f * (d__1 = mn7int_1.x[in - 1], abs(d__1))
		;
/* Computing MAX */
	d__1 = gsmin, d__2 = mn7int_1.dirin[in - 1] * .1f;
...
/*                   if parameter has limits */
	if (mn7inx_1.nvarl[*k - 1] > 1) {
	    if (mn7der_1.gstep[in - 1] > .5f) {
		mn7der_1.gstep[in - 1] = .5f;
	    }
...
/* Subroutine */ int mnpars_(char *crdbuf, integer *icondn, ftnlen crdbuf_len)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___806 = { 1, celmnt, 0, "(BN,F20.0)", 20, 1 };
	...
	pinti)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___845 = { 0, chbufi, 0, "(I4)", 4, 1 };
	...
	ftnlen chpt_len)
{
    ...
    static char cdot[1] = ".";
    static char cslash[1] = "/";
    static char cblank[1] = " ";
    ...
    /* Fortran I/O blocks */
    static cilist io___888 = { 0, 0, 0, "(18X,A)", 0 };
    static cilist io___889 = { 0, 0, 0, "(1X,G14.7,A,A)", 0 };
    static cilist io___890 = { 0, 0, 0, "(18X,A)", 0 };
    static cilist io___893 = { 0, 0, 0, "(12X,12G10.4)", 0 };
    static cilist io___895 = { 0, 0, 0, "(25X,A,G13.7,A)", 0 };
...
/* Subroutine */ int mnprin_(integer *inkode, doublereal *fval)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___900 = { 0, 0, 0, "(A)", 0 };
    static cilist io___905 = { 0, 0, 0, "(/A,A)", 0 };
    static icilist io___907 = { 0, cheval, 0, "(G15.7)", 15, 1 };
    static icilist io___909 = { 0, chedm, 0, "(E10.2)", 10, 1 };
    static cilist io___911 = { 0, 0, 0, fmt_905, 0 };
    static cilist io___913 = { 0, 0, 0, fmt_907, 0 };
    static cilist io___916 = { 0, 0, 0, fmt_908, 0 };
    static cilist io___923 = { 0, 0, 0, fmt_910, 0 };
    static cilist io___925 = { 0, 0, 0, fmt_911, 0 };
    static cilist io___931 = { 0, 0, 0, fmt_952, 0 };
    static icilist io___934 = { 0, cx2, 0, "(G14.5)", 14, 1 };
    static icilist io___935 = { 0, cx3, 0, "(G14.5)", 14, 1 };
    static cilist io___936 = { 0, 0, 0, fmt_952, 0 };
    static cilist io___937 = { 0, 0, 0, fmt_1004, 0 };
    static cilist io___938 = { 0, 0, 0, "(I4,1X,A11,G14.5,A,2G14.5)", 0 };
    static cilist io___939 = { 0, 0, 0, "(I4,1X,A11,G14.5,A)", 0 };
    static cilist io___940 = { 0, 0, 0, "(31X,A,G10.3)", 0 };
...
/*    when INKODE=5, MNPRIN chooses IKODE=1,2, or 3, according to ISW(2) */
    ...
    if (mn7npr_1.nu == 0) {
	io___900.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___900);
...
/*              print title if Minos errors, and title exists. */
    if (ikode == 4 && s_cmp(mn7tit_1.ctitl, mn7tit_1.cundef, (ftnlen)50, (
	    ftnlen)10) != 0) {
	io___905.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___905);
...
/*              report function value and status */
    ...
    io___911.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___911);
	...
	dcmax = 1.f;
	dc = min(mn7min_1.dcovar,dcmax) * 100.f;
	io___916.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___916);
	do_fio(&c__1, chedm, (ftnlen)10);
	do_fio(&c__1, (char *)&mn7cnv_1.istrat, (ftnlen)sizeof(integer));
...
L20:
    ...
    ncol = 3;
    io___923.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___923);
    i__1 = ncol;
    for (kk = 1; kk <= i__1; ++kk) {
...
/*              variable parameter. */
	...
	if (ikode == 1) {
	    if (mn7inx_1.nvarl[i__ - 1] <= 1) {
		io___931.ciunit = mn7iou_1.isyswr;
		s_wsfe(&io___931);
		do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	...
	io___936.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___936);
	do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	do_fio(&c__1, cnambf, (ftnlen)11);
...
/*               check if parameter is at limit */
	...
	if ((d__1 = cos(mn7int_1.x[l - 1]), abs(d__1)) < .001f) {
	    io___937.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___937);
	    e_wsfe();
	}
	goto L200;
...
/* Subroutine */ int mnpsdf_(void)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___947 = { 0, chbuff, 0, "(I3)", 3, 1 };
    static icilist io___949 = { 0, chbuff, 0, "(E12.2)", 12, 1 };
    static cilist io___957 = { 0, 0, 0, fmt_550, 0 };
    static cilist io___958 = { 0, 0, 0, fmt_551, 0 };
    static icilist io___960 = { 0, chbuff, 0, "(G12.5)", 12, 1 };
...
/*                    Store VHMAT in P, make sure diagonal pos. */
	...
	s[i__ - 1] = 1.f / sqrt(mn7var_1.vhmat[ndexd - 1]);
	i__4 = i__;
	for (j = 1; j <= i__4; ++j) {
	    ++ndex;
/* L213: */
	    mn7sim_1.p[i__ + j * 50 - 51] = mn7var_1.vhmat[ndex - 1] * s[i__ 
		    - 1] * s[j - 1];
	}
    }
...
/* Computing MAX */
	...
	io___957.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___957);
	e_wsfe();
	io___958.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___958);
	i__4 = mn7npr_1.npar;
	for (ip = 1; ip <= i__4; ++ip) {
	...
	ndex = ip * (ip + 1) / 2;
/* L216: */
	mn7var_1.vhmat[ndex - 1] *= padd + 1.f;
    }
    s_copy(mn7tit_1.cstatu, "NOT POSDEF", (ftnlen)10, (ftnlen)10);
    s_wsfi(&io___960);
	...
	y, integer *jh, integer *jl)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___966 = { 0, 0, 0, fmt_1000, 0 };
...
/* C */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L10: */
	mn7sim_1.p[i__ + *jh * 50 - 51] = pnew[i__];
    }
    y[*jh] = *ynew;
...
/* L25: */
	...
	i__2 = nparp1;
	for (j = 2; j <= i__2; ++j) {
	    if (mn7sim_1.p[i__ + j * 50 - 51] > pbig) {
		pbig = mn7sim_1.p[i__ + j * 50 - 51];
	    }
	    if (mn7sim_1.p[i__ + j * 50 - 51] < plit) {
		plit = mn7sim_1.p[i__ + j * 50 - 51];
	    }
/* L30: */
	}
...
L40:
    return 0;
L45:
    io___966.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___966);
    do_fio(&c__1, (char *)&mn7npr_1.npar, (ftnlen)sizeof(integer));
    e_wsfe();
    goto L40;
	...
	futil)
{
    ...
    static char cpromt[40*3] = " ENTER MINUIT TITLE, or \"SET INPUT n\" : " 
    ...
    integer s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void);
    /* Subroutine */ int s_copy(char *, char *, ftnlen, ftnlen);
    integer s_rsfe(cilist *), e_rsfe(void), i_indx(char *, char *, ftnlen, 
	    ftnlen);
    ...
    /* Fortran I/O blocks */
    static cilist io___973 = { 0, 0, 0, "(A)", 0 };
    static cilist io___975 = { 1, 0, 1, "(A)", 0 };
    static cilist io___979 = { 0, 0, 0, "(A,I3)", 0 };
    static cilist io___980 = { 0, 0, 0, fmt_21, 0 };
    static cilist io___981 = { 0, 0, 0, "(A,I3)", 0 };
    static cilist io___983 = { 0, 0, 0, "(A,A/)", 0 };
    static cilist io___984 = { 0, 0, 0, "(1X,A50)", 0 };
    static cilist io___985 = { 0, 0, 0, "(1X,78(1H*))", 0 };
    static cilist io___987 = { 0, 0, 0, "(A)", 0 };
    static cilist io___988 = { 0, 0, 0, "(A)", 0 };
    static cilist io___989 = { 0, 0, 0, "(4X,75(1H*))", 0 };
    static cilist io___991 = { 0, 0, 0, fmt_405, 0 };
    static cilist io___992 = { 0, 0, 0, "(1H ,10(1H*))", 0 };
    static cilist io___994 = { 1, 0, 1, fmt_420, 0 };
...
L10:
    if (mn7flg_1.isw[5] == 1) {
	io___973.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___973);
    ...
    io___975.ciunit = mn7iou_1.isysrd;
    i__1 = s_rsfe(&io___975);
    if (i__1 != 0) {
	goto L100001;
    }
...
/*                                           . .   preemptive commands */
	...
	io___979.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___979);
	...
	++mn7flg_1.icomnd;
	io___980.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___980);
	do_fio(&c__1, (char *)&mn7flg_1.icomnd, (ftnlen)sizeof(integer));
...
/*              if title is 'SET TITLE', skip and read again */
    ...
    io___984.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___984);
    do_fio(&c__1, mn7tit_1.ctitl, (ftnlen)50);
    e_wsfe();
...
/*          format error */
	...
	if (mn7flg_1.isw[5] == 1) {
	    ...
	    io___988.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___988);
...
/*                                        . . . . . . . . . . set covar */
	...
	do_fio(&c__1, crdbuf, (ftnlen)50);
	e_wsfe();
	io___992.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___992);
	e_wsfe();
	npar2 = mn7npr_1.npar * (mn7npr_1.npar + 1) / 2;
	io___994.ciunit = mn7iou_1.isysrd;
	i__1 = s_rsfe(&io___994);
	if (i__1 != 0) {
	    goto L100002;
	}
	i__2 = npar2;
...
/* C    If IOPT=0, sets only MINOS errors to undefined */
	...
	mn7min_1.fval3 = abs(mn7min_1.amin) * 2.f + 1.f;
	mn7min_1.edm = mn7cns_1.bigedm;
	mn7flg_1.isw[3] = 0;
	mn7flg_1.isw[1] = 0;
	mn7min_1.dcovar = 1.f;
...
/* Subroutine */ int mnsave_(void)
{
    ...
    /* Subroutine */ int s_copy(char *, char *, ftnlen, ftnlen);
    integer s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void),
	     s_rsfe(cilist *), e_rsfe(void), f_open(olist *), f_rew(alist *);
    ...
    /* Fortran I/O blocks */
    static cilist io___1002 = { 0, 0, 0, fmt_32, 0 };
    static cilist io___1003 = { 0, 0, 0, fmt_35, 0 };
    static cilist io___1004 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1005 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1007 = { 0, 0, 0, fmt_37, 0 };
    static cilist io___1008 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1010 = { 1, 0, 0, "(10HSET TITLE )", 0 };
    static cilist io___1011 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1012 = { 0, 0, 0, "(10HPARAMETERS)", 0 };
    static cilist io___1016 = { 0, 0, 0, fmt_1001, 0 };
    static cilist io___1017 = { 0, 0, 0, fmt_1001, 0 };
    static cilist io___1018 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1019 = { 1, 0, 0, fmt_1003, 0 };
    static cilist io___1021 = { 0, 0, 0, fmt_1004, 0 };
    static cilist io___1023 = { 0, 0, 0, fmt_501, 0 };
    static cilist io___1024 = { 0, 0, 0, fmt_502, 0 };
    static cilist io___1025 = { 0, 0, 0, "(A,I4)", 0 };
    static cilist io___1026 = { 0, 0, 0, "(A,I4,A)", 0 };
    static cilist io___1027 = { 0, 0, 0, "(A,I4)", 0 };
    static cilist io___1028 = { 0, 0, 0, "(A)", 0 };
...
/* C */
	...
	io___1002.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1002);
...
/*                new file, open it */
	io___1003.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1003);
	do_fio(&c__1, (char *)&mn7iou_1.isyssa, (ftnlen)sizeof(integer));
	e_wsfe();
	if (mn7flg_1.isw[5] == 1) {
	    ...
	    e_wsfe();
	    io___1005.ciunit = mn7iou_1.isysrd;
	    s_rsfe(&io___1005);
...
/*                               file is now correctly opened */
    if (mn7flg_1.isw[5] == 1) {
	io___1007.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1007);
	do_fio(&c__1, (char *)&mn7iou_1.isyssa, (ftnlen)sizeof(integer));
	e_wsfe();
	io___1008.ciunit = mn7iou_1.isysrd;
	s_rsfe(&io___1008);
...
/*                               and rewound if requested */
    io___1010.ciunit = mn7iou_1.isyssa;
    i__1 = s_wsfe(&io___1010);
    if (i__1 != 0) {
	goto L700;
    }
    i__1 = e_wsfe();
    ...
    io___1011.ciunit = mn7iou_1.isyssa;
    s_wsfe(&io___1011);
    do_fio(&c__1, mn7tit_1.ctitl, (ftnlen)50);
    e_wsfe();
...
/*         parameter without limits */
	io___1016.ciunit = mn7iou_1.isyssa;
	s_wsfe(&io___1016);
	do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	do_fio(&c__1, mn7nam_1.cpnam + (i__ - 1) * 10, (ftnlen)10);
	do_fio(&c__1, (char *)&mn7ext_1.u[i__ - 1], (ftnlen)sizeof(doublereal)
...
L100:
	io___1017.ciunit = mn7iou_1.isyssa;
	s_wsfe(&io___1017);
	do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	do_fio(&c__1, mn7nam_1.cpnam + (i__ - 1) * 10, (ftnlen)10);
	do_fio(&c__1, (char *)&mn7ext_1.u[i__ - 1], (ftnlen)sizeof(doublereal)
...
/*                                  write out covariance matrix, if any */
    ...
    nlines += ncovar;
    io___1023.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1023);
    ...
    e_wsfe();
    if (ncovar > 0) {
	io___1024.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1024);
...
/* Subroutine */ int mnscan_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___1047 = { 0, 0, 0, fmt_1001, 0 };
    static cilist io___1049 = { 0, 0, 0, fmt_1000, 0 };
...
/* C        retains the best function and parameter values found. */
    ...
    ncall = (integer) (mn7arg_1.word7[1] + .01f);
    if (ncall <= 1) {
	ncall = 41;
    }
    if (ncall > 101) {
    ...
    iparwd = (integer) (mn7arg_1.word7[0] + .1f);
    ipar = max(iparwd,0);
...
/*         finished with scan of parameter IPAR */
    ...
    mnexin_(mn7int_1.x);
    io___1047.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1047);
...
/* Subroutine */ int mnseek_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___1053 = { 0, 0, 0, fmt_3, 0 };
    static cilist io___1071 = { 0, 0, 0, fmt_601, 0 };
...
/* C */
    ...
    if (mn7flg_1.isw[4] >= 1) {
	io___1053.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1053);
...
/*              set up step sizes, starting values */
	...
	mn7int_1.dirin[ipar - 1] = alpha * 2.f * mn7err_1.werr[ipar - 1];
	if (mn7inx_1.nvarl[iext - 1] > 1) {
/*              parameter with limits */
...
/*                              search loop */
	...
	for (ipar = 1; ipar <= i__2; ++ipar) {
	    ...
	    mnrn15_(&rnum2, &iseed);
/* L100: */
	    mn7int_1.x[ipar - 1] = xmid[ipar - 1] + (rnum1 + rnum2 - 1.f) * 
		    .5f * mn7int_1.dirin[ipar - 1];
	}
	mninex_(mn7int_1.x);
...
L600:
    if (mn7flg_1.isw[4] > 1) {
	io___1071.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1071);
...
/* Subroutine */ int mnset_(S_fp fcn, U_fp futil)
{
    ...
    static char cname[10*30] = "FCN value " "PARameters" "LIMits    " "COVar"
    ...
    static char cprlev[34*5] = "-1: NO OUTPUT EXCEPT FROM \"SHOW\"  " " 0: R"
	    "EDUCED OUTPUT                " " 1: NORMAL OUTPUT               "
	    "  " " 2: EXTRA OUTPUT FOR PROBLEM CASES" " 3: MAXIMUM OUTPUT    "
	    "            ";
    static char cstrat[44*3] = " 0: MINIMIZE THE NUMBER OF CALLS TO FUNCTION" 
	    " 1: TRY TO BALANCE SPEED AGAINST RELIABILITY" " 2: MAKE SURE MI"
	    "NIMUM TRUE, ERRORS CORRECT  ";
    static char cdbopt[40*7] = "REPORT ALL EXCEPTIONAL CONDITIONS       " 
    ...
    extern /* Subroutine */ int mnexin_(doublereal *), mnrset_(integer *), 
	    mnlims_(void), mnwerr_(void), mnwarn_(char *, char *, char *, 
	    ftnlen, ftnlen, ftnlen), mnamin_(S_fp, U_fp), mnprin_(integer *, 
	    doublereal *), mnmatu_(integer *);
    static integer igrain, iswsav;
    extern /* Subroutine */ int mnpsdf_(void);

    /* Fortran I/O blocks */
    static cilist io___1083 = { 0, 0, 0, "(A/)", 0 };
    static cilist io___1086 = { 0, 0, 0, fmt_151, 0 };
    static cilist io___1091 = { 0, 0, 0, fmt_289, 0 };
    static cilist io___1092 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1093 = { 0, 0, 0, "(27X,A)", 0 };
    static cilist io___1094 = { 0, 0, 0, fmt_1061, 0 };
    static cilist io___1095 = { 0, 0, 0, fmt_1081, 0 };
    static cilist io___1096 = { 0, 0, 0, fmt_1082, 0 };
    static cilist io___1097 = { 0, 0, 0, fmt_1091, 0 };
    static cilist io___1101 = { 0, 0, 0, fmt_1002, 0 };
    static cilist io___1102 = { 0, 0, 0, fmt_1111, 0 };
    static cilist io___1103 = { 0, 0, 0, fmt_1121, 0 };
    static cilist io___1105 = { 0, 0, 0, fmt_1141, 0 };
    static cilist io___1108 = { 0, 0, 0, fmt_1151, 0 };
    static cilist io___1110 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___1111 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1112 = { 0, 0, 0, "(20X,A)", 0 };
    static cilist io___1113 = { 0, 0, 0, fmt_1175, 0 };
    static cilist io___1115 = { 0, 0, 0, "(1X,A)", 0 };
    static cilist io___1116 = { 0, 0, 0, "(A,I3)", 0 };
    static cilist io___1117 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1119 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1120 = { 0, 0, 0, "(A,E12.3)", 0 };
    static cilist io___1121 = { 0, 0, 0, "(A,I4)", 0 };
    static cilist io___1122 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___1124 = { 0, 0, 0, fmt_1286, 0 };
    static cilist io___1125 = { 0, 0, 0, fmt_1901, 0 };
    static cilist io___1126 = { 0, 0, 0, fmt_2101, 0 };
    static cilist io___1128 = { 0, 0, 0, "(' ABOVE COMMAND IS ILLEGAL.   IGN"
	    "ORED')", 0 };
...
/*        Keep approximate covariance matrix, even if new param value */
    ...
    goto L4000;
L25:
    io___1083.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1083);
...
L90:
    ...
    i__1 = mn7npr_1.npar;
    for (i__ = 1; i__ <= i__1; ++i__) {
	mn7err_1.ern[i__ - 1] = 0.f;
/* L95: */
	mn7err_1.erp[i__ - 1] = 0.f;
    }
    mnwerr_();
    goto L4000;
/*                                        . . . . . . . . . . set input */
...
L150:
    ...
    mnrn15_(&val, &jseed);
    if (mn7flg_1.isw[4] > 0) {
	io___1086.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1086);
...
/*                                        . . . . . . . . . . set epsmac */
L210:
    if (mn7arg_1.word7[0] > 0. && mn7arg_1.word7[0] < .1f) {
...
/* L285: */
    ...
    goto L4000;
L288:
    io___1091.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1091);
...
L1060:
    ...
    io___1092.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1092);
...
L1100:
    ...
    io___1101.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1101);
...
/*                                       . . . . . . . . . . show width */
L1110:
    io___1102.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1102);
...
L1150:
    ...
    mnrn15_(&val, &igrain);
    ikseed = igrain;
    io___1108.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1108);
...
/*                                        . . . . . . . show strategy */
L1170:
    io___1111.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1111);
...
/*                                            . . . . . show page throw */
L1190:
    io___1116.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1116);
...
/*                                        . . . . . . . . . show epsmac */
L1210:
    io___1120.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1120);
...
/*                              UNKNOWN COMMAND */
L1900:
    io___1125.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1125);
    do_fio(&c__1, mn7tit_1.cword, (ftnlen)20);
    e_wsfe();
    goto L2100;
...
/* Subroutine */ int mnsimp_(S_fp fcn, U_fp futil)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___1140 = { 0, 0, 0, fmt_100, 0 };
    static cilist io___1167 = { 0, 0, 0, "(A)", 0 };
...
/* C */
    ...
    wg = 1.f / (real) mn7npr_1.npar;
    if (mn7flg_1.isw[4] >= 0) {
	io___1140.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1140);
...
L8:
	...
	for (k = 1; k <= i__2; ++k) {
/* L9: */
	    mn7sim_1.p[k + i__ * 50 - 51] = mn7int_1.x[k - 1];
	}
/* L10: */
    }
    jh = nparp1;
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L20: */
	mn7int_1.x[i__ - 1] = mn7sim_1.p[i__ + jl * 50 - 51];
    }
    mninex_(mn7int_1.x);
    s_copy(mn7tit_1.cstatu, "PROGRESS  ", (ftnlen)10, (ftnlen)10);
    if (mn7flg_1.isw[4] >= 1) {
...
/* L59: */
	...
	mn7sim_1.pbar[i__ - 1] = pb - wg * mn7sim_1.p[i__ + jh * 50 - 51];
/* L60: */
	mn7sim_1.pstar[i__ - 1] = (alpha + 1.f) * mn7sim_1.pbar[i__ - 1] - 
		alpha * mn7sim_1.p[i__ + jh * 50 - 51];
    }
...
/*         point * better than jl, calculate new point ** */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L61: */
	mn7sim_1.pstst[i__ - 1] = gamma * mn7sim_1.pstar[i__ - 1] + (1.f - 
		gamma) * mn7sim_1.pbar[i__ - 1];
    }
...
/*         try a parabola through ph, pstar, pstst.  min = prho */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L64: */
	mn7sim_1.prho[i__ - 1] = rho * mn7sim_1.pbar[i__ - 1] + (1.f - rho) * 
		mn7sim_1.p[i__ + jh * 50 - 51];
    }
...
L73:
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L74: */
	mn7sim_1.pstst[i__ - 1] = beta * mn7sim_1.p[i__ + jh * 50 - 51] + (
		1.f - beta) * mn7sim_1.pbar[i__ - 1];
    }
    mninex_(mn7sim_1.pstst);
    (*fcn)(&nparx, mn7der_1.gin, &ystst, mn7ext_1.u, &c__4, (U_fp)futil);
    ++mn7cnv_1.nfcn;
...
L76:
    if (mn7flg_1.isw[4] >= 0) {
	io___1167.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1167);
...
L78:
    if (mn7flg_1.isw[4] >= 0) {
	io___1168.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1168);
...
/* L82: */
    ...
    if (mn7min_1.edm > mn7min_1.epsi * 2.f) {
	goto L1;
    }
L90:
    if (mn7flg_1.isw[4] >= 0) {
...
/* Subroutine */ int mnstin_(char *crdbuf, integer *ierr, ftnlen crdbuf_len)
{
    ...
    /* Subroutine */ int s_copy(char *, char *, ftnlen, ftnlen);
    integer s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void),
	     s_rsfi(icilist *), e_rsfi(void), f_inqu(inlist *), s_rsfe(cilist 
	    *), e_rsfe(void), f_open(olist *), f_rew(alist *);
    ...
    /* Fortran I/O blocks */
    static cilist io___1178 = { 0, 0, 0, "(A,A)", 0 };
    static icilist io___1179 = { 1, cunit__, 0, "(BN,F10.0)", 10, 1 };
    static cilist io___1183 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___1187 = { 0, 0, 0, fmt_132, 0 };
    static cilist io___1188 = { 0, 0, 0, fmt_135, 0 };
    static cilist io___1189 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1190 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1191 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1192 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1193 = { 0, 0, 0, fmt_137, 0 };
    static cilist io___1194 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1196 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___1197 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1199 = { 0, 0, 0, fmt_290, 0 };
    static cilist io___1200 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1201 = { 0, 0, 0, "(A,A)", 0 };
    static cilist io___1202 = { 0, 0, 0, fmt_601, 0 };
...
L130:
    ...
    noname = FALSE_;
    io___1183.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1183);
...
L131:
	...
	if (noname) {
	    ...
	    io___1187.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___1187);
...
/*                new file, open it */
	io___1188.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1188);
	do_fio(&c__1, (char *)&iunit, (ftnlen)sizeof(integer));
	e_wsfe();
	if (noname) {
	    ...
	    do_fio(&c__1, " PLEASE GIVE FILE NAME:", (ftnlen)23);
	    e_wsfe();
	    io___1191.ciunit = mn7iou_1.isysrd;
	    s_rsfe(&io___1191);
	    do_fio(&c__1, cfname, (ftnlen)64);
	    e_rsfe();
	}
...
L136:
    ...
    io___1193.ciunit = mn7iou_1.isyswr;
    s_wsfe(&io___1193);
    do_fio(&c__1, (char *)&iunit, (ftnlen)sizeof(integer));
    e_wsfe();
    io___1194.ciunit = mn7iou_1.isysrd;
    s_rsfe(&io___1194);
    do_fio(&c__1, canswr, (ftnlen)1);
    e_rsfe();
...
L300:
    if (mn7io2_1.nstkrd >= 10) {
	io___1200.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1200);
	do_fio(&c__1, " INPUT FILE STACK SIZE EXCEEDED.", (ftnlen)32);
	e_wsfe();
	goto L800;
	...
	n, integer *ifail)
{
    ...
    /* Local variables */
    static integer i__, j, k;
    static doublereal q[50], s[50], si, pp[50];
	...
	copt_len, ftnlen corg_len, ftnlen cmes_len)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___1217 = { 0, 0, 0, "(A,A/A,A)", 0 };
    static cilist io___1218 = { 0, 0, 0, "(A,A/A,A)", 0 };
    static cilist io___1222 = { 0, 0, 0, "(/1X,I5,A,A,A,A/)", 0 };
    static cilist io___1224 = { 0, 0, 0, "(A,I2,A)", 0 };
    static cilist io___1225 = { 0, 0, 0, "(A)", 0 };
    static cilist io___1227 = { 0, 0, 0, "(1X,I6,1X,A,1X,A)", 0 };
    static cilist io___1228 = { 0, 0, 0, "(1H )", 0 };
...
/*             Either print warning or put in buffer */
	...
	ityp = 1;
	if (mn7log_1.lwarn) {
	    io___1217.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___1217);
	...
	ityp = 2;
	if (mn7log_1.lrepor) {
	    io___1218.ciunit = mn7iou_1.isyswr;
	    s_wsfe(&io___1218);
...
L200:
	...
	io___1222.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1222);
	...
	io___1225.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1225);
...
/* L300: */
	...
	io___1228.ciunit = mn7iou_1.isyswr;
	s_wsfe(&io___1228);
	e_wsfe();
    }
    return 0;
...
/*                         calculate external error if v exists */
	...
	for (l = 1; l <= i__1; ++l) {
		...
		du1 = al + (sin(mn7int_1.x[l - 1] + dx) + 1.f) * .5f * ba - 
			mn7ext_1.u[i__ - 1];
		du2 = al + (sin(mn7int_1.x[l - 1] - dx) + 1.f) * .5f * ba - 
			mn7ext_1.u[i__ - 1];
		if (dx > 1.f) {
		    du1 = ba;
		}
...
/*                          global correlation coefficients */
	...
	i__1 = mn7npr_1.npar;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    mn7err_1.globcc[i__ - 1] = 0.f;
	    k1 = i__ * (i__ - 1) / 2;
	    i__2 = i__;
	    for (j = 1; j <= i__2; ++j) {
		k = k1 + j;
		mn7sim_1.p[i__ + j * 50 - 51] = mn7var_1.vhmat[k - 1];
/* L130: */
		mn7sim_1.p[j + i__ * 50 - 51] = mn7sim_1.p[i__ + j * 50 - 51];
	    }
	}
	...
	if (ierr == 0) {
		...
		denom = mn7sim_1.p[iin + iin * 50 - 51] * mn7var_1.vhmat[
			ndiag - 1];
		if (denom <= 1. && denom >= 0.) {
		    mn7err_1.globcc[iin - 1] = 0.f;
		} else {
		    mn7err_1.globcc[iin - 1] = sqrt(1.f - 1.f / denom);
		}
/* L150: */
	    }
	}
